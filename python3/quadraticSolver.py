    ...
    def __init__(self, xvalues = [0, 100, 50], yvalues = [0, 0, 0]):
        ...
        for x in self.xvalues:
            if type(x).__name__ != "str":
                ...
                return
        # make sure all the input values are decimal type so that we will not get floating point errors
        for i in range(len(self.yvalues)):
            self.yvalues[i] = decimal.Decimal(self.yvalues[i])
            self.xvalues[i] = decimal.Decimal(self.xvalues[i])
        if self.xvalues[0] != 0:
